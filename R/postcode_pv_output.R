...
postcode_pv_output <- function(Postcode, MonthDay,data.folder,InstalledPV,temp_vector){
  ...
  for(i in 2:length(a)){
    tvalue <- loadToEnv(paste0(a[i],"/POA.",Postcode,".RData"))[[paste0("POA.",Postcode)]];
    tvalue <- data.frame(tvalue)
    c<-tvalue[which(grepl(MonthDay,tvalue$datetime)==1),]
  ...
  solar_hourly_vector[is.na(solar_hourly_vector)] <- 0
  
  solar_hourly_vector[,19] <- 0
  
  
  
  means <- as.numeric(solar_hourly_vector[1,])
  
  standard_deviations <- as.numeric(solar_hourly_vector[2,])
